...
@section('breadcrumbs', Breadcrumbs::render('guardian.teachers'))
@section('content')
    <div class="row">
        <div class="col">
            <div class="card">
                ...
                <div class="card-header">
                    ...
                    <a href="{{ route('guardian.teachers.create') }}" class="btn btn-primary">Add Data</a>
                </div>
                            ...
                            @foreach ($teachers as $teacher)
                                <tr>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="m-r-10 rounded-circle">
                                                @empty($teacher->image)
                                                    <a href="{{ route('guardian.teachers.show', $teacher->id) }}"
                                                        class="btn p-2 text-center rounded-circle btn-{{ randomColor($teacher->id) }}"
                                                        style="width: 40px;">
                                                        {{ substr($teacher->fullName(), 0, 2) }}
                                                    </a>
                                                @else
                                                    <a class="" href="{{ route('guardian.teachers.show', $teacher->id) }}">
                                    ...
                                    <td class="d-flex justify-content-around">
                                        ...
                                        <a href="{{ route('guardian.teachers.edit', $teacher->id) }}"
                                            class="btn btn-sm btn-info text-white" title="edit">
                                            <i class="fas fa-pencil-alt"></i>
                                        </a>
                                        <a href="{{ route('guardian.teachers.destroy', $teacher->id) }}"
                                        ...
                                        <form id="destroy-item{{ $teacher->id }}"
                                            action="{{ route('guardian.teachers.destroy', $teacher->id) }}" method="POST"
                                            class="d-none">
                                            @csrf
                                            @method('DELETE')
                                        </form>
