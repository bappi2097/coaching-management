...
@section('breadcrumbs', Breadcrumbs::render('student.students'))
@section('content')
    <div class="row">
        <div class="col">
            <div class="card">
                ...
                <div class="card-header">
                    ...
                    <a href="{{ route('student.students.create') }}" class="btn btn-primary">Add Data</a>
                </div>
                            ...
                            @foreach ($students as $student)
                                <tr>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="m-r-10 rounded-circle">
                                                @empty($student->image)
                                                    <a href="{{ route('student.students.show', $student->id) }}"
                                                        class="btn p-2 text-center rounded-circle btn-{{ randomColor($student->id) }}"
                                                        style="width: 40px;">
                                                        {{ substr($student->fullName(), 0, 2) }}
                                                    </a>
                                                @else
                                                    <a class="" href="{{ route('student.students.show', $student->id) }}">
                                    ...
                                    <td class="d-flex justify-content-around">
                                        ...
                                        <a href="{{ route('student.students.edit', $student->id) }}"
                                            class="btn btn-sm btn-info text-white" title="edit">
                                            <i class="fas fa-pencil-alt"></i>
                                        </a>
                                        <a href="{{ route('student.students.destroy', $student->id) }}"
                                        ...
                                        <form id="destroy-item{{ $student->id }}"
                                            action="{{ route('student.students.destroy', $student->id) }}" method="POST"
                                            class="d-none">
                                            @csrf
                                            @method('DELETE')
                                        </form>
