...
@section('breadcrumbs', Breadcrumbs::render('student.results'))
@section('content')
    <div class="row">
        <div class="col">
            <div class="card">
                            ...
                            @foreach ($results as $result)
                                <tr>
                                        ...
                                        <div class="d-flex align-items-center">
                                            <div class="m-r-10 rounded-circle">
                                                @empty($result->student->image)
                                                    <a href="{{ route('student.students.show', $result->student->id) }}"
                                    ...
                                    {{-- <td class="d-flex justify-content-around">
                                        ...
                                        <a href="{{ route('student.results.edit', $result->id) }}"
                                            class="btn btn-sm btn-info text-white" title="edit">
                                            <i class="fas fa-pencil-alt"></i>
                                        </a>
                                        <a href="{{ route('student.results.destroy', $result->id) }}"
                                        ...
                                        <form id="destroy-item{{ $result->id }}"
                                            action="{{ route('student.results.destroy', $result->id) }}" method="POST"
                                            class="d-none">
                                            @csrf
                                            @method('DELETE')
                                        </form>
