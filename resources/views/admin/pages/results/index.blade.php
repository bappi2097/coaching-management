...
@section('breadcrumbs', Breadcrumbs::render('officer.results'))
@section('content')
    <div class="row">
        <div class="col">
            <div class="card">
                ...
                <div class="card-header">
                    ...
                    <a href="{{ route('officer.results.create') }}" class="btn btn-primary">Add Data</a>
                </div>
                            ...
                            @foreach ($results as $result)
                                <tr>
                                        ...
                                        <div class="d-flex align-items-center">
                                            <div class="m-r-10 rounded-circle">
                                                @empty($result->student->image)
                                                    <a href="{{ route('officer.students.show', $result->student->id) }}"
                                    ...
                                    <td class="d-flex justify-content-around">
                                        <a href="{{ route('officer.results.show', $result->id) }}"
                                            class="btn btn-sm btn-success text-white" title="show">
                                            <i class="far fa-eye"></i>
                                        </a>
                                        <a href="{{ route('officer.results.edit', $result->id) }}"
                                            class="btn btn-sm btn-info text-white" title="edit">
                                            <i class="fas fa-pencil-alt"></i>
                                        </a>
                                        <a href="{{ route('officer.results.destroy', $result->id) }}"
                                        ...
                                        <form id="destroy-item{{ $result->id }}"
                                            action="{{ route('officer.results.destroy', $result->id) }}" method="POST"
                                            class="d-none">
                                            @csrf
                                            @method('DELETE')
                                        </form>
