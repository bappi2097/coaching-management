...
@section('breadcrumbs', Breadcrumbs::render('guardian.exams.edit', $exam->id))
@section('content')
    <div class="row">
        <div class="col">
            <div class="card">
                ...
                <div class="card-header">
                    ...
                    <a href="{{ route('guardian.exams.index') }}" class="btn btn-primary">Back</a>
                </div>
                <div class="card-body">

                    <form action="{{ route('guardian.exams.update', $exam->id) }}" method="POST"
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group @error('exam_type_id') has-danger @enderror">
                                    <label for="exam_type_id" class="form-control-label">Full Name</label>
                                    <select class="form-control @error('exam_type_id') is-invalid @enderror"
                                        ...
                                        @foreach (\App\Models\ExamType::get() as $index => $examType)
                                            <option value="{{ $examType->id }}"
                                                {{ selected($examType->id, $exam->examType->id) }}>
                                                {{ $index + 1 }} . {{ $examType->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('exam_type_id')
                                    ...
                                    <select class="form-control @error('course_id') is-invalid @enderror" id="course_id"
                                        ...
                                        @foreach (\App\Models\Course::get() as $index => $course)
                                            <option value="{{ $course->id }}"
                                                {{ selected($course->id, $exam->course->id) }}>
                        ...
                        <div class="form-group @error('marks') has-danger @enderror">
                            ...
                            <input class="form-control @error('marks') is-invalid @enderror" type="number"
                                value="{{ $exam->marks }}" id="marks" name="marks">
                            @error('marks')
                                <span class="invalid-feedback" role="alert">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="form-group @error('exam_date') has-danger @enderror">
                            ...
                            <input class="form-control @error('exam_date') is-invalid @enderror" type="datetime-local"
                                value="{{ date('Y-m-d\TH:i:s', strtotime($exam->exam_date)) }}" name="exam_date"
                                id="exma_date">
                            @error('exam_date')
                                <span class="invalid-feedback" role="alert">{{ $message }}</span>
                            @enderror
